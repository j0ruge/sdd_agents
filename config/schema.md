...
| `ON_ESCALATION_CMD` | *(empty)* | The pager. After attempting the durable ledger write, the runner executes this shell command — `bash -c`, stdin closed, output to stderr — on **every escalation that ends the run with rc 3**: `increment-blocked`, `dirty-tree`, `handoff-blocked`, `app-down`, `session-died`, `budget-exhausted`, `no-progress`, `no-work`, `retry-gate-red`, `hat-crossed`, `kit-touched`. A hook can therefore read its corresponding row when persistence succeeded; the existing ledger warning remains authoritative when it did not. Its environment carries `SDD_REASON` (the ledger `kind`), `SDD_PHASE`, `SDD_MISSION`, `SDD_PROJECT` and `SDD_GATE_WHY` (the gate's sentence). Empty ⇒ the line stops in silence, as it did before L6 of the 2026-09-03 audit — which measured zero notification sites in the runner and a human learning the line had stopped by watching `tail -F`. The projection (`--dry-run`) never runs it. The entire hook family gets five seconds, then TERM, then KILL after one more second; a local subreaper adopts and cancels background children even after `setsid`, and checkout ownership remains held until they are reaped; failure or timeout warns without replacing the escalation's rc. If `timeout(1)` is unavailable, the runner warns and skips the hook instead of running it without a bound. `review-to-draft` is a `degraded` row, not an rc 3, and gets no pager. Example: `ON_ESCALATION_CMD='notify-send "sdd $SDD_MISSION" "$SDD_PHASE stopped: $SDD_REASON"'`. |
