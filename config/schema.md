...
| `ON_ESCALATION_CMD` | *(empty)* | The pager. A shell command the runner executes — `bash -c`, stdin closed, output to stderr — on **every escalation that ends the run with rc 3**: `increment-blocked`, `dirty-tree`, `handoff-blocked`, `app-down`, `budget-exhausted`, `no-progress`, `no-work`, `hat-crossed`, `kit-touched`. Its environment carries `SDD_REASON` (the ledger `kind`), `SDD_PHASE`, `SDD_MISSION`, `SDD_PROJECT` and `SDD_GATE_WHY` (the gate's sentence). Empty ⇒ the line stops in silence, as it did before L6 of the 2026-09-03 audit — which measured zero notification sites in the runner and a human learning the line had stopped by watching `tail -F`. The projection (`--dry-run`) never runs it; a hook that exits non-zero is a warning, never a second failure, because the escalation is already in the journal and the ledger. `review-to-draft` is a `degraded` row, not an rc 3, and gets no pager. Example: `ON_ESCALATION_CMD='notify-send "sdd $SDD_MISSION" "$SDD_PHASE stopped: $SDD_REASON"'`. |
