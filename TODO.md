...
- [ ] **`gate_QA` aceita relatório de QA de OUTRA missão** — `bin/sdd:1138` — a Âncora 1 pega o
...
- [ ] **Citação NÃO-cercada acima do cabeçalho ainda vira o gênero do bug** — `bin/sdd:1204` — o
...
- [ ] **O carimbo de mutação cobre 4 dos 8 caminhos que a sandbox do catálogo copia** —
  `bin/sdd:1610` contra `tests/check-mutation.sh:4666` — a chave lê `bin tests templates config`,
...
- [ ] **O braço 1 da guarda de forma do `ledger_repo_root` não tem probe, e a mutação junta os
  dois** — `bin/sdd:2600` (`case "$gitdir" in`) — sob o shim pré-2.31 o valor não começa com `/`, então quem dispara é
...
- [ ] **O piso do shim pré-2.31 prova que o shim é um git falso, não que o runner o consulta** —
  `tests/check-autonomy.sh:4251` — o piso invoca `git` diretamente sob o `PATH` do shim, e nada
...
- [ ] **A sonda de `--path-format` do preflight não tem asserção nenhuma** —
  `bin/sdd:4560` — a guarda que ela anuncia (`ledger_repo_root` recusando a resposta de duas
...
- [ ] **O `moved2` do `cmd_kaizen` não tem asserção que morra ao apagá-lo** —
  `bin/sdd:7287` — a asserção `covered:` do `moved` cobre a primeira atribuição; neutralizar a
...
- [ ] **A linha `N kit agent(s) checked` não é observável por nenhum fixture** — `bin/sdd:4783` —
...
- [ ] **O fixture de `stream-json` não tem checagem de proveniência** — `tests/check-autonomy.sh:129`
  ...
  registra o comando, mas `health_provenance` (`bin/sdd:1420`) só confere as 3 fixtures de skill
...
- [ ] **`.sdd/logs/` não tem poda e agora guarda o stream inteiro** — `bin/sdd:734` — desde o I10
...
- [ ] **`check-autonomy.sh` é vermelho intermitente, causa desconhecida** — `bin/sdd:4381` —
...
- [ ] **A economia de `current_phase()`/`next_pending_phase()` depende da memoização e ninguém
  conta** — `bin/sdd:1757` vs `:675` — as duas reavaliam o gate de toda fase a cada
...
- [ ] **O schema da série não tem sensor de drift contra a prosa que o descreve** — `bin/sdd:8425`
...
- [ ] **O `gate:` do frontmatter só é cobrado quando existe, e 6 das 14 rodadas não o têm** —
  `bin/sdd:1374` — a recusa de placeholder no `gate:` deixa AUSENTE em paz de propósito, para não
...
- [ ] **O `gate_QA` compra o placeholder do próprio template como evidência de jornada** —
  `bin/sdd:1132` — em projeto sem interface a única âncora é `frontmatter gate`, testada só por
...
- [ ] **O ramo de forma do `score:` no `cmd_health` não tem asserção nem mutante** —
  `bin/sdd:5143` — é ele que impede que um `score:` presente e ilegível caminhe até `ok` e carimbe:
...
- [ ] **O piso do catálogo mora só no consumidor; quem imprime o `score:` segue sem nenhum** —
  `tests/check-mutation.sh:4947` — com `CATALOG=()` o laço roda zero vezes, `errors` fica 0 e o
  arquivo imprime `score: 0 caught, 0 known gap(s), of 0` saindo 0. O F1 pôs o piso no `cmd_health`,
  hoje o único chamador — mas duas frases do próprio runner (`bin/sdd:5095` e `:5108`) mandam o
...
- [ ] **`FIXME` e `XXX` são recusados pelo `gate_REVIEW` sem nenhum mundo que prove** —
  `bin/sdd:1422` — a lista de palavras de preenchimento tem sete entradas e só cinco têm mundo no
...
- [ ] **A guarda de vazio do `mutation_stamp_key` só cobre a ausência TOTAL dos quatro caminhos** —
  `bin/sdd:1684` — com `tests/` presente e `bin/` ausente, o `find` imprime o que achou, sai não-zero,
...
- [ ] **O braço `else ""` da célula do laço de revisão não tem fixture** — `bin/sdd:8290` — a
...
- [ ] **A proveniência do `sdd health` lê "a mais nova em cache", não "a que roda"** —
  `bin/sdd:5456` — o `report-template.md` da `codereview` sai de `find … | sort -V | tail -1` sobre
...
- [ ] **Fase interrompida depois do REVIEW faz o pipeline REGREDIR para o REVIEW** —
  `bin/sdd:1520` (`git status --porcelain`) — o `gate_REVIEW` reprova com árvore suja e não distingue "o revisor deixou
...
- [ ] **Lixo ignorado pelo git dentro dos quatro diretórios move a chave do carimbo** —
  `bin/sdd:1678` (`mutation_stamp_key`) — a chave é `find -type f` sobre a árvore, não sobre o que o git rastreia: um
...
- [ ] **`sdd kaizen` recusa rodar de um worktree do próprio kit** — `bin/sdd:9185` — a porta
...
- [ ] **Nenhuma chave de caminho do `.sdd/config.sh` é normalizada antes de virar padrão de `case`**
  — `bin/sdd:1944` — `hat_expand` troca `$TODO_FILE` **literalmente** no `writes:` do chapéu, lido contra
...
- [ ] **`run_phase` cria o diretório de log da sessão sem guarda nenhuma** — `bin/sdd:3778` (`mkdir -p`) — é a
...
- [ ] **Uma sessão escreve o ledger com o `bin/sdd` que tinha em MEMÓRIA ao ser lançada** —
  `bin/sdd:3533` — a missão que ACRESCENTA um campo é a única que não o registra (3 de 4 rodadas
...
- [ ] **35% do `docs/pipeline.md` é um subsistema só, e ele cresce toda missão do ledger** —
  `docs/pipeline.md:929` — as seções `The autonomy ledger` (323 linhas) e `The kaizen loop` (174)
...
- [ ] **`turns` não aparece em nenhuma view humana** — `bin/sdd:3533` — o campo está na tabela de
...
- [ ] **`sdd adr check` só fala texto + rc; não existe `--json`** — `bin/sdd:6444` (`check)`) — quem quiser
...
- [ ] **A linha `kit-touched` do ledger afirma uma atribuição que o runner nunca mediu** —
  `bin/sdd:2933` — o `warn` da tela ressalva (*"if that was you working on the kit in another
...
- [ ] **`gate_EXEC` valida por uma leitura e conta por outra, e uma célula vazia as separa** —
  `bin/sdd:1008` — o laço lê com `IFS=$'\t' read`, que COLAPSA tabs por serem whitespace de IFS; o
...
- [ ] **O caminho histórico infere `M` quando a memória está vazia, e M é o maior valor possível** —
  `bin/sdd:2906` — sem memória o braço `advanced` passa a ser satisfeito por qualquer prosa que não
...
- [ ] **A regra `doing` conta como pendente não tem probe, e sem ela o `gate_EXEC` fecha a fase por
  cima de um incremento em voo** — `bin/sdd:542` — degradar `$4 == "pending" || $4 == "doing"` para
...
- [ ] **A metade `repo` da chave de memória do caminho histórico não tem probe** — `bin/sdd:2906` —
...
- [ ] **A guarda `$r.phase == "EXEC"` do `historic_progress` não tem probe e falha na direção da
  lisonja** — `bin/sdd:2906` — removida, a suíte fica verde e uma linha NÃO-EXEC que passou o gate
...
- [ ] **`sdd approve` commita na branch corrente, mesmo a padrão, e só o `00-missao.md`** —
  `bin/sdd:6602` — medido aqui: o commit da aprovação caiu na `main` local, com o `01-plano.md`, o
...
- [ ] **O 2º Python do worker custa ~30 ms em toda chamada coordenada** — `bin/sdd:9489` — o
...
<!-- sdd:decided -->
- **O `sdd preflight` não provaria que a sessão headless executa comando** — refutado: `bin/sdd:4564` manda rodar `bash -c 'echo sdd-preflight-ok'` sob as flags do `run_phase` desde `2083680`, e sob o chapéu do executor desde 2026-09-06 (2026-09-25)
...
- **O stub do `sdd adr new` manda escrever em `OUTPUT_LANG`, e o `check-lang.sh` lê `docs/adr/` como inglês** — limite declarado pela D15, não achado: `bin/sdd:6168` (2026-09-25)
