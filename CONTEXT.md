...
- `docs/adr/` ainda não está na `surface()` do `tests/check-lang.sh` (lista enumerada + piso de 26 caminhos) — decidir no plano se o I13.3 a inclui.
- A sessão kaizen ganha linha própria no ledger? Com qual `mission` (a missão ainda não existe quando ela roda)?
