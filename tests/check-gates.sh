...
#
# The six regimes below are DIFFERENTIAL pairs: each demands the text of the branch it is in AND
...
#
# They run LAST on purpose. The claude stub planted at the top of this file exists to turn any real
# session into a loud failure, and this block has to replace it with one that answers; nothing
# below inherits the fixture state these lines leave behind.
echo "== sdd close: the artifact decides =="

CLOSE_CTL="$FIX/.sdd/logs/close-ctl"           # what the acli stub answers: done|notdone|error0|error1
...
# the exit code discriminates nothing, and `.[]?.key` is the only witness there is.
#
...
#!/usr/bin/env bash
printf '%s\n' "\$*" >> "$CLOSE_ACLI_LOG"
case "\$(cat "$CLOSE_CTL" 2>/dev/null)" in
...
# close_run <acli verdict> <session rc> — CALLED, never substituted: it publishes its results in
# globals, and a `$(close_run …)` would run it in a subshell where every one of those assignments
# dies with the substitution. That is a scar this repo already carries (CLAUDE.md, the one-shot
# guard of autonomy_kit_stamp), and it is why the two outputs come back as CLOSE_RC_OUT/CLOSE_OUT.
CLOSE_RC_OUT=0
CLOSE_OUT=""
close_run() {
...
# 2. THE INCIDENT. The session exits 0 having only asked for confirmation; the issue is still open.
close_run notdone 0
...
# 3. Confirmed by the issue itself.
close_run "done" 0
assert_eq "close: with the issue actually Done it says closed, and the journal records what confirmed it" \
  "rc:0 confirms:1 refuses:0 journal:1" \
  "rc:$CLOSE_RC_OUT confirms:$(has "$CLOSE_OUT" 'JIRA confirms') refuses:$(has "$CLOSE_OUT" 'does not confirm') journal:$(has "$(cat "$CLOSE_JOURNAL" 2>/dev/null || true)" 'verified=true')"

# 4. The artifact outranks the exit code in BOTH directions — a session that fell over after
...
# 5. Prose where JSON was expected, under BOTH exit codes. Falling closed here is a false NEGATIVE
#    on expired auth — the safe direction, and regime 1's message is what tells the human so.
close_run error1 0
C5_RC1="$CLOSE_RC_OUT"; C5_OUT1="$CLOSE_OUT"
close_run error0 0
assert_eq "close: an acli answering prose instead of JSON fails closed, whichever exit code it picks" \
  "rc1:1 confirms1:0 refuses1:1 rc0:1 confirms0:0 refuses0:1" \
  "rc1:$C5_RC1 confirms1:$(has "$C5_OUT1" 'JIRA confirms') refuses1:$(has "$C5_OUT1" 'does not confirm') rc0:$CLOSE_RC_OUT confirms0:$(has "$CLOSE_OUT" 'JIRA confirms') refuses0:$(has "$CLOSE_OUT" 'does not confirm')"

# 6. Control. With JIRA off the command asks nothing of anyone — and the two `:0` terms are the
...
rm -f "$CLOSE_MARK" "$CLOSE_ACLI_LOG"
C6_RC=0
C6_OUT="$( cd "$FIX" && "$SDD" close "$MISSION" 2>&1 )" || C6_RC=$?
assert_eq "close: with JIRA off nothing is asked of anyone — no session, no acli, no verdict" \
  "rc:0 nothing:1 session-spent:0 acli-called:0" \
  "rc:$C6_RC nothing:$(has "$C6_OUT" 'nothing to close') session-spent:$(spent) acli-called:$(if [ -e "$CLOSE_ACLI_LOG" ]; then printf 1; else printf 0; fi)"
sed -i 's/^JIRA_ENABLED=false$/JIRA_ENABLED=true/' "$FIX/.sdd/config.sh"
