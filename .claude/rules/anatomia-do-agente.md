...
`config_read_key` é a única leitura de chave fora do `load_config` e diz "does not parse";
`test_cmd_lists_only` é o predicado único do `--list` (health 2b e preflight); e o `_fail` do
preflight só rebaixa a `warn` quando o runner **diz** que falta o manifesto na raiz
(`test_cmd_missing_manifest`). A âncora do `TODO.md` também virou sensor (ADR 0011).
