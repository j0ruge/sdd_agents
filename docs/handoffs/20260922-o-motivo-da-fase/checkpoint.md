...
| I5 | kind no-work: célula ilegível para a linha antes da sessão (portas 1 e 3) + contrato do kind | `o=$(bash tests/check-autonomy.sh 2>&1); grep -c '^  ok    door 1: an unreadable cell stops the line before any session' <<< "$o"` → `1` | pending | — |
| I6 | mesmo motivo duas vezes para a linha (forma a) + porta 2 no retry inline | `o=$(bash tests/check-autonomy.sh 2>&1); grep -c '^  ok    door 1: the same reason twice stops the line as no-work' <<< "$o"` → `1` | pending | — |
