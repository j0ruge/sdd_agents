...
| I2 | os três leitores do ledger filtram por repo | `bash tests/check-kaizen.sh 2>&1 \| grep -c 'a row from another repo never enters the series'` → `1` | pending | — |
| I3 | preflight compara conteúdo do agente, não presença | `bash tests/check-preflight.sh 2>&1 \| grep -c 'a drifted agent copy fails the preflight'` → `1` | pending | — |
| I4 | aviso de branch base alcança run e kaizen | `bash tests/check-gates.sh 2>&1 \| grep -c 'the base branch warning reaches sdd run'` → `1` | pending | — |
