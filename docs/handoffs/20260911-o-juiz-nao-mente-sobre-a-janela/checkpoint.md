...
| R6 | r2 #1 — `cmd_close` joga stderr dentro do `.jsonl` que ele mesmo parseia (`2>&1`), e uma linha de stderr faz o `jq` abortar: a linha de `close` perde custo, turns, cache e harness em silêncio | `o=$(bash tests/check-autonomy.sh 2>&1); grep -c '^  ok    close keeps stderr out of the stream it parses' <<< "$o"` → `1` ou mais | pending | — |
