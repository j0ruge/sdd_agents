...
- 2026-09-25 02:24 · `EXEC` · I10 `a8a65db`: regra no `check_file`. Primeiro efeito medido: as próprias edições do incremento empurraram a âncora do item do #70 (`tests/check-todo.sh:1190`) e o lint a apontou — re-ancorada para `:1611` no mesmo commit, como a regra manda. Item sem crase nenhuma fica com a regra 3 (sem contar duas vezes). Fixtures do `--check` ganharam arquivos ancorados ao lado (`bsym`/`fsym`/`xsym`, box vira repo git), nunca chave de desligar. Sabotagens: apagar a chamada no `check_file` → rc 92; contar item sem crase duas vezes → rc 92.
- 2026-09-25 02:31 · `EXEC` · I11 `a646115`: 7 `RESOLVED by`; fusão do item dos "demais templates" no "`templates/` é single-language" (o sobrevivente ganhou o precedente `todo.<lang>.md`). Desvio do plano: o achado do stub do `sdd adr new` **não** entrou como item — o `bin/sdd:6108` o declara limite D15 ("It is not a finding by the D15 rule") desde `004ed1d`; virou registro decidido. Catraca 90 → 89 (esperado pelo plano: 100 − 1 + 1 ± re-ancoragem; real: 100 − 7 (I8) − 3 (I9) − 1 = 89). KAIZEN_LOG e gaveta (#53) atualizados.
