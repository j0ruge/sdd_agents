...
| I3 | Escopo repo: `SPEC_DIR`, specs SpecKit, número solto, duplicata, stub não preenchido + 1 mutante | `o=$(bash tests/check-adr.sh 2>&1); grep -c '^  ok    a bare ADR 0042 in a spec with no file fails' <<< "$o"` → `1` | done | `PENDING` |
