...
- 2026-09-17 · `I5` · `GATE_ADR_WARN_WHY` **não** entrou aqui: global sem leitor é SC2034, e o `shellcheck` já cobrou a mesma coisa duas vezes nesta missão (I1: `ADR_LINK_RE`; I3: `mode`). Chega no I6 junto com quem o lê.
- 2026-09-17 · `I5` · o early-return de `off` no `adr_gate_verdict` **sobrevive a toda sabotagem hoje**: no gate, `off` e `warn` são indistinguíveis (os dois derivam a fase). A diferença é a linha de ledger do `warn`, que chega no I6 — o probe que fecha a lacuna se chama `off writes no degraded row`. Declarado no cabeçalho do sensor, nunca apagado por "não consegui quebrar".
- 2026-09-17 · `I6` · ⚠️ **gasto acidental de US$ 1,01 nesta sessão**: reproduzi o caminho `dirty-tree` à mão, num repo temporário, **sem o stub `claude` no PATH** — 4 sessões opus reais. O fixture do sensor sempre teve o stub; o erro foi meu, fora dele. Lição: repro manual de caminho de `sdd run` monta o stub ANTES, como o `check-gates.sh` faz por construção.
- 2026-09-17 · `I6` · dois defeitos de fixture achados por probe vermelho, não por raciocínio: (1) a célula Commit do checkpoint é lida **verbatim** — crase em volta do sha fez o gate dizer "commit não existe" e a corrida escalou pelo motivo errado; (2) `grep -c` imprime NADA em arquivo ausente e imprime `0` **e sai 1** quando não casa, então tanto `|| true` quanto `|| echo 0` dão contagem errada.
- 2026-09-17 · `I6` · TRÊS regras do `cmd_run` sobrevivem a toda sabotagem, todas **medidas** e declaradas no cabeçalho do sensor com o mundo que não consegui construir: o one-shot (o pré-check de EXEC é entrado no máximo uma vez por corrida em qualquer mundo que um probe alcance), a guarda de `--dry-run` (medido: a projeção nunca chega ao sítio) e o reset do marcador (armado e lido dentro de uma derivação só).
- 2026-09-17 · `I7` · a sabotagem achou que o `hat_expand` **não tinha leitor**: apagar a substituição de `$ADR_DIR` deixava `check-hat.sh` e `check-preflight.sh` verdes. O planner era o único chapéu a declará-lo e o PLAN não passa por `run_phase`. Dois consertos: o `sdd-docs` também declara (DOCS passa, e o agente já lista `docs/adr/*` como alvo de drift), e um probe chama `hat_expand` direto, com os dois termos — substituição que APAGA o placeholder (glob virando `/**`) fica tão vermelha quanto uma que ignora.
- 2026-09-17 · `I7` · a asserção do plano `^  ok    adr check:` **nunca casaria**: o `sdd preflight` imprime pelo `ok()` do runner, que é `  ok` + TRÊS espaços, não os quatro do `pass()` dos sensores. Corrigida na escrita.
- 2026-09-17 · `I7` · `off` no preflight não conta ADRs de propósito: quem conta é a varredura de repo, e um segundo contador aqui seria uma segunda resposta para "quantos ADRs" — a forma que o I3 já pagou. `adr_check_repo` publica `ADR_COUNT`.
- 2026-09-17 · `I8` · um item novo no `TODO.md` (`--json`, sem consumidor) e a baseline 97 → 98 no MESMO commit — é isso que faz o número se mover num diff com autor. ⚠️ a baseline está dentro da chave do carimbo de mutação, então o `sdd health` do I9 só vale rodado DEPOIS deste commit.
- 2026-09-17 · `I9` · carimbo escrito: `score: 317 caught, 0 known gap(s), of 317` — exatamente a métrica 3 do `00-missao.md` (309 + 8). `sdd health` = `kit healthy`, 7 linhas verdes.
- 2026-09-17 · `I9` · ⚠️ `sdd health --release` dá **2 de 6**, não 6 de 6 como o plano previu. As quatro vermelhas são **pré-existentes** e têm dono no ADR 0007 (segundo repo, origem das skills, missão ponta a ponta no segundo repo, LICENSE + identificadores de cliente). A linha que esta missão possui é a **6**, que estava `stale` e ficou verde.
- 2026-09-17 · `I9` · verificação ponta a ponta do plano, passos 3 e 4: clone descartável → `adr new --dry-run` devolve `docs/adr/0009-probe.md`, escopo de repo rc 0, apagar a linha `Spec:` do 0008 faz o `sdd why … PLAN` nomear o back-link **e** o remédio. Cópia do repo-alvo com `warn`+`SPEC_DIR=specs` → **rc 0**: 40 ADRs, 16 missões contadas, **19 specs sem linha ADR** contadas como `info`, `specs/023/adr/` intocado, nada escrito no repo real. Os dois `ADR 0030` soltos das specs de lá apontam para arquivo que **existe** — a regra ficou calada por não haver o que achar, e o probe do sensor prova que ela dispara quando falta.
