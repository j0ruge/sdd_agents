...
| 2026-09-23 | planning RS-890 (DSR API — other repo, graph of 2026-09-14) | Who depends on `ColaboratorRepository`? | `explain "ColaboratorRepository"` | 25 lines | `grep -rln ColaboratorRepository src` → service, repo, 1 test | 1 grep, 3 lines | **complete**; the graph predates the branch, so `findByEmail` is absent, as expected |
| 2026-09-23 | idem | Where are routes mounted through `conditionalRoute`? | `explain "conditionalRoute"` | 12 lines | `grep -c 'conditionalRoute(' src/server.ts` → 20 | 1 grep | **out-of-scope**, as the rules predict: 20 module-level calls, not function-to-function, so only the `import` edge exists |
