...
A REVIEW session that commits — or leaves dirty — a file outside its hat's `writes:` **stops the
line**: `HAT-CROSSED` in `.sdd/logs/<mission>/pipeline.log`, a `hat-crossed` row in the ledger after
the session's own row, rc 3, and the pager. The reviewer's `writes:` is the mission directory plus
`TODO_FILE` (principle 5) and, in the kit, `tests/health-baseline.txt`.
...
`BLOCKED`, or a draft PR when `PUBLISH_ON_REVIEW_BLOCKED=draft` — one of the two places the runner
lowers its **own** bar instead of stopping, and it records the fact once per run
(`event:"degraded"`, kind `review-to-draft`, in the ledger's field reference below). The other is
`ADR_CHECK=warn`, which derives EXEC with the mission's `adr:` unsatisfied and writes one
`adr-check` row per run — the instrument of a migration rather than of a failure.
...
`hat_crossed_escalation` reads that marker — and the kit guard's `KIT_TOUCHED_WHY` — at four
doors, always after the session's ledger row: `sdd run`'s first pass, its inline retry, `sdd retry`
and `sdd close`; rc 3, a `hat-crossed` (or `kit-touched`) row, and the pager.
...
exits 0 with the right list and the guard arms correctly. The diff only becomes uncomputable once
something has since dropped that object — `git gc --prune=now`, after which `git diff` exits 128
with `bad object`.

`grep -c HAT-CROSSED .sdd/logs/<mission>/pipeline.log` answering `0` is what a healthy
...
`.sdd/logs/<mission>/`. Before it, every phase `sdd run` **derives** gets a line of its own —
`<ts>  PHASE  <X>  reason="<the gate's sentence>"` — written before the session is paid for:
the session line says only `rc=0`, which reads as success over a session that had nothing to do
(issue #56, `20260922-o-motivo-da-fase`). A phase forced with `--phase` gets no `PHASE` line —
nothing was derived. The two files:
...
prompt a phase would get, without a session — it derives the phase first, so its `Why this phase:`
line is the one the session would read, and that runs the gates (`TEST_CMD`, and `E2E_CMD` once
the derivation reaches QA), exactly like `sdd status` — and the `boot bill` line of `sdd census` is the
...
There are **four row shapes**: a *session* row (`event:"session"`), an *escalation* row — any row
that spent no session because something went wrong, `event:"blocked"` or `event:"degraded"` —,
since `20260831-a-rodada-que-andou`, a *recorded gate closure* (`event:"gate_pass"`), which also
spends no session but records the pipeline moving **forward** for free, and, since 2026-09-11, a
*recorded ticket closure* (`event:"close"`): `sdd close` buys a real paid session — same
`--allowedTools`, same `--permission-mode`, same `--max-budget-usd` as any phase — and until then it
was the one door that spent one and wrote nothing here, so every close the kit ever ran was
invisible to the judge that counts `launches` off this file. It is deliberately **not** a session
row wearing `phase: "CLOSE"`: CLOSE is not a phase, it has no gate and `current_phase()` never
derives it, so a row carrying `moved` and `gate` would enter the judge's arithmetic as a graded
phase that ran and said nothing. It is written **only** where a session was actually spent — the
arm that finds the issue already Done returns before `claude` is launched (its journal line says
`session=none`), and a closure nobody paid for is not a row. Escalation and gate-closure
...
| `rc` | integer \| `null` | on escalation rows | The `claude` process's exit code. `null` when the session log carried none. Present on `close` rows too, and there it decides **nothing**: it is one exit code shared by the session that closed the issue and the session that only asked about it, which is the label `cmd_close` stopped believing on 2026-08-25. |
| `issue` | string | on every shape but `close` | The ticket key the close session was launched against, read from `issue:` in `10-ticket.md`. |
...
| `session_error` | string \| `null` | on escalation rows, on `event:"gate_pass"` rows and on `event:"close"` rows — only a row that SPENT a session can carry one | **Why the session died, when the harness said why.** Read by `run_phase` off the same distilled `result` object `cost_usd` and `turns` come out of, truncated to 200. The rule is written POSITIVELY and is narrow: `is_error == true` **and** `result` a non-empty string. Measured 2026-09-18 over the 11 sessions with `is_error: true` in the four target repos — 2 carry a cause (`subtype: "success"`, `"Failed to authenticate: OAuth session expired and could not be refreshed"`), 3 are `error_max_budget_usd` with `result: null` (the kit's **designed** loop, which must not stop the line) and 6 are `error_during_execution` with `result: null` (died, cause unknown). So a rule anchored on "an error subtype" would miss the first regime and fire on the second; only the pair separates all three. The 6 unknown deaths carry `null` here and escalate nothing — *unknown escalates nothing*, the same literal rule `app_probe` keeps. Written as `not <the budget ceiling>` the field would concede the **fourth** regime by omission the day the harness invents one. A non-`null` value here is what arms `kind: "session-died"` below; before `20260918-a-sessao-morreu-e-o-gate-levou-a-culpa` such a session reached the ledger as `kind: "no-progress"`, which this file classes as pure friction and D12/D16 read as the pipeline spinning — an expired token filed as the kit's own waste. |
...
| `gate_why` | string, truncated to 200 characters | on `event:"gate_pass"` rows | The gate's stated reason (on an escalation row, the reason the phase was not satisfied when the runner gave up on it). ⚠️ It was `never` until `20260831-a-rodada-que-andou` added the third shape, and the exception is not an omission: the gate that closed was evaluated inside `current_phase()`, which runs as `$( )`, so `GATE_WHY` died with the subshell — and re-running the gate here to recover a sentence would run `TEST_CMD` a second time for prose. The two rows above this one are the whole reason this column exists: a shape added to the code and not to the table is a reader trusting `never` over a key that is not there. |
...
human reader closes an arithmetic over **seven** buckets (comparable session, non-comparable
session, escalation, recorded gate closure, **ticket closure**, **judge row** — the `sdd kaizen`
sessions that leave the axis but not the header — and unrecognized) and prints a line for each that
is non-empty, so a row nobody counts leaves the header total without leaving a trace — which is
`unrecognized` with the alarm switched off. `tests/check-autonomy.sh` closes that sum over all
seven. ⚠️ The number in this sentence has already rotted three times (r2 finding #6 and r3 finding #6 of the 2026-09-11 judge mission, plus the comment in `bin/sdd` beside the code):
a bucket added without the sentence is drift by construction, so count them off the `as $…`
bindings in `cmd_autonomy`, never off this line.
...
The US$ cell of **both** views is session money **plus the money the recorded closures cost**
(`cost_usd` on `event:"close"` rows). Recording that money and never spending it was the state
until this mission: a mission with a US$ 2 phase and a US$ 3 close reported `US$ 2.00`, in the one
view D12 reads US$-per-merged-PR off. A closure still grades nothing — it is absent from
`session(s)`, from the three outcomes, from `launch(es)` and from `reopened` — the distinction
`$closes` has drawn since it got a bucket of its own.
⚠️ The two views group ONE population by **different keys**, so a closure can be attributable in
one and orphaned in the other: a closure whose mission has a comparable session but whose `kit_sha`
has none lands on the `--by-mission` line and on no line of the table. Whatever matched no printed
line is carried as an explicit remainder on the closures bucket (`US$ N of it on no line above`),
which is what keeps the two totals equal whichever key they grouped by — and is asserted over a
fixture built for exactly that asymmetry, never over one where both views happen to agree.
